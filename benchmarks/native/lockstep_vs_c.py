...
The two kernels are timed in **alternating rounds** (``--rounds``, default 9)
rather than one to completion and then the other, and the round order is swapped
on odd rounds. Each arena is 1-6 MiB, so the two do not stay cache-resident
together and whichever kernel ran second used to inherit the state the first left
behind -- a fixed order baked that bias into every run. Measured on the previous
fixed-order driver, swapping which kernel went first moved
``telemetry_filter_aggregation``'s mean ratio from 1.036 to 0.832. The reported
per-tick time is the median across rounds; ``*_per_tick_us_min`` carries the
fastest round so a noisy run is visible rather than silently folded into the
ratio.

Interleaving cannot fix *cross-process* variation, which on some hosts is
substantial for the cache-resident C copy loops: within one process both kernels
are stable (median within a few percent of min), but the C baseline can settle
into a different mode from run to run. Read each ratio as a band across several
invocations, not a single number.
...
#define CHUNK ((ITERS + ROUNDS - 1) / ROUNDS)
...
/* Time CHUNK calls of one kernel and return nanoseconds per call. */
static double time_chunk(void (*kernel)(uint8_t*), uint8_t* arena) {{
    double t0 = bench_now_ns();
    for (int it = 0; it < CHUNK; ++it) kernel(arena);
    return (bench_now_ns() - t0) / (double)CHUNK;
}}

static void tick_shim(uint8_t* arena) {{ Lockstep_Tick(arena); }}
...
int main(void) {{
    ...
    /* Interleave the two kernels in alternating rounds rather than timing one
       to completion and then the other. Each arena is 1-6 MiB, so two of them
       do not stay resident together: whichever kernel runs second inherits the
       cache state the first left behind, and a fixed order bakes that bias into
       every run. Swapping which kernel leads on odd rounds makes the schedule
       symmetric, and spreading both across the whole measurement window shares
       any drift (frequency, a noisy neighbour on a shared host) between them
       instead of charging it to whoever ran last. Per-round medians then
       discard a round that got descheduled. */
    double ls_samples[ROUNDS];
    double c_samples[ROUNDS];
    for (int r = 0; r < ROUNDS; ++r) {{
        if (r % 2 == 0) {{
            ls_samples[r] = time_chunk(tick_shim, a_ls);
            c_samples[r]  = time_chunk(c_reference, a_c);
        }} else {{
            c_samples[r]  = time_chunk(c_reference, a_c);
            ls_samples[r] = time_chunk(tick_shim, a_ls);
        }}
    ...
    double cs_ls = checksum(a_ls), cs_c = checksum(a_c);
    double ls_min = ls_samples[0], c_min = c_samples[0];
    for (int r = 1; r < ROUNDS; ++r) {{
        if (ls_samples[r] < ls_min) ls_min = ls_samples[r];
        if (c_samples[r] < c_min) c_min = c_samples[r];
    }}
    double ls_ns = median(ls_samples, ROUNDS);
    double c_ns  = median(c_samples, ROUNDS);

    printf("{{\\"lockstep_ns\\": %.3f, \\"c_ns\\": %.3f, "
           "\\"lockstep_ns_min\\": %.3f, \\"c_ns_min\\": %.3f, \\"rounds\\": %d, "
           "\\"checksum_lockstep\\": %.6f, \\"checksum_c\\": %.6f}}\\n",
           ls_ns, c_ns, ls_min, c_min, ROUNDS, cs_ls, cs_c);
...
def _run_workload(
    ...
    return {
        ...
        # Fastest round for each kernel. On a shared host the median absorbs
        # interference while the minimum is the closest thing to an
        # uncontended sample; a large median/min gap means the run was noisy
        # and the ratio deserves less weight.
        "lockstep_per_tick_us_min": round(float(measured["lockstep_ns_min"]) / 1000.0, 4),
        "c_per_tick_us_min": round(float(measured["c_ns_min"]) / 1000.0, 4),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=9,
        help=(
            "Alternating measurement rounds per workload; the reported per-tick "
            "time is the median across rounds (default: 9). Each round times "
            "iterations/rounds calls of each kernel, swapping which kernel leads "
            "on odd rounds so neither is systematically favoured by cache state."
        ),
    )
    ...
    if args.rounds > args.iterations:
        raise SystemExit("--rounds cannot exceed --iterations")
    ...
    print(
        f"Lockstep vs hand-written C ({args.iterations} iters each over {args.rounds} "
        f"interleaved rounds, median reported, "
