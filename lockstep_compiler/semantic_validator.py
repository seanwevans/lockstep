...
def build_semantic_validator(base_visitor_cls):
    class LockstepSemanticValidator(base_visitor_cls):
        ...
        def _call_ctx(self, ctx, name: str, default=None):
        ...
        def _token_text(token) -> str:
            return token.getText() if hasattr(token, "getText") else str(token)

        def _collect_id_tokens(self, ctx):
            id_method = getattr(ctx, "ID", None)
            if not callable(id_method):
                return []
            try:
                id_tokens = id_method()
            except TypeError:
        ...
        def _resolve_lvalue_type(self, ctx, *, for_write: bool = False):
            path = tuple(
                self._token_text(token) for token in self._collect_id_tokens(ctx)
            )
            return self._resolve_ast_lvalue_type(path, ctx, for_write=for_write)

        def _record_kernel_signature(
            self, ctx, target: dict[str, list[SemanticKernelParam]]
        ):
            name = self._token_text(ctx.ID())
            params = []
            param_list = self._call_ctx(ctx, "paramList")
            if param_list:
                for param in param_list.param():
                    declared_type = param.typeName().getText()
                    self._validate_declared_type(
                        declared_type, param.typeName(), "LCK310"
                    )
                    params.append(
                        SemanticKernelParam(
                            name=self._token_text(param.ID()),
        ...
        def _expr_list_args(self, expr_list_ctx) -> list[Any]:
            if expr_list_ctx is None:
                return []
            expr_method = getattr(expr_list_ctx, "expr", None)
            if not callable(expr_method):
                return []
            args = expr_method() or []
        ...
        def _format_operand_types(self, *types: str) -> str:
            return f"[{', '.join(types)}]"

        def _report_invalid_operand_types(
            self,
            ctx,
            op: str,
            expectation: str,
            *actual_types: str,
            ...
            if message is None:
                message = (
                    ...
                    f"but got {self._format_operand_types(*actual_types)}."
                )
            self._add_diagnostic(
                severity="error",
                code=code,
        ...
        def _resolve_expr_type(self, expr):
            ...
            declared_type = getattr(expr, "declared_type", None)
            ...
            def _binary(
                method_name: str, default_op: str, expectation: str, predicate, result
            ):
                children = _children(method_name)
                if children is None or len(children) < 2:
                    return None
                left_type = self._resolve_expr_type(children[0])
                right_type = self._resolve_expr_type(children[1])
                if left_type is None or right_type is None:
                    return None
                op = default_op
                get_child = getattr(expr, "getChild", None)
                if callable(get_child):
                    try:
                        op = get_child(1).getText()
                    except Exception:
                        op = default_op
                if predicate(left_type, right_type):
                    return result(left_type, right_type)
                self._report_invalid_operand_types(
                    expr, op, expectation, left_type, right_type
                )
                return None

            mul_children = _children("mulExpr")
            if mul_children is not None and len(mul_children) >= 2:
                left_type = self._resolve_expr_type(mul_children[0])
                right_type = self._resolve_expr_type(mul_children[1])
                op = "+"
                get_child = getattr(expr, "getChild", None)
                if callable(get_child):
                    try:
                ...
                if left_type in numeric_types and right_type in numeric_types:
                    ...
                    return None
                self._report_invalid_operand_types(
                    expr, op, "numeric", left_type, right_type
                )
                return None

            logical_children = _children("bitwiseOrExpr")
            if logical_children is not None and len(logical_children) >= 2:
                return _binary(
                    "bitwiseOrExpr",
                    "&&",
                    "bool",
                    lambda left, right: left == "bool" and right == "bool",
                    lambda _left, _right: "bool",
                )

            bitwise_or_children = _children("bitwiseXorExpr")
            if bitwise_or_children is not None and len(bitwise_or_children) >= 2:
                return _binary(
                    "bitwiseXorExpr",
                    "|",
                    "integer",
                    lambda left, right: left in integer_types and right == left,
                    lambda left, _right: left,
                )

            bitwise_and_children = _children("equalityExpr")
            if bitwise_and_children is not None and len(bitwise_and_children) >= 2:
                return _binary(
                    "equalityExpr",
                    "&",
                    "integer",
            ...
            unary = self._call_ctx(expr, "unaryExpr")
            if unary is not None:
                operand_type = self._resolve_expr_type(unary)
                ...
                if callable(get_child):
                    try:
                        op = get_child(0).getText()
                    except Exception:
                        op = "!"
                if op == "!":
                    if operand_type == "bool":
                        return "bool"
                    self._report_invalid_operand_types(expr, op, "bool", operand_type)
                    return None
                if op == "-":
                    if operand_type in numeric_types:
                        return operand_type
                    self._report_invalid_operand_types(
                        expr, op, "numeric", operand_type
                    )
                    return None
                return operand_type

            lvalue = self._call_ctx(expr, "lvalue")
            if lvalue is not None:
                return self._resolve_lvalue_type(lvalue)

            id_token = self._call_ctx(expr, "ID")
            if id_token is not None:
                name = self._token_text(id_token)
                expr_list = self._call_ctx(expr, "exprList")
                if expr_list is None:
                    ...
                    if symbol is None:
                        ...
                        return None
                    if symbol.kind == "local" and not self._is_symbol_assigned(name):
                        self._report_use_before_definition(name, expr)
                        return None
                    self._mark_symbol_used(name)
                    return symbol.declared_type
                args = self._expr_list_args(expr_list)
                if name in {"int", "float", "double", "uint", "bool"}:
                    return name if len(args) == 1 else None
                if name == "select":
                    if len(args) != 3:
                        self._add_diagnostic(
                            severity="error",
                            ...
                            message=f"Built-in 'select(...)' expects 3 arguments, but got {len(args)}.",
                            ctx=expr,
                    ...
                    if condition_type is not None and condition_type != "bool":
                        self._add_diagnostic(
                            ...
                            message=f"Type mismatch for built-in 'select' condition: expected bool, got {condition_type}.",
                            ctx=expr,
                ...
                signature = self.pure_functions.get(name)
                if signature is None:
                    self._add_diagnostic(
                        severity="error",
                        code=SEMANTIC_DIAGNOSTIC_CODES["pure_unknown_function"],
                        message=f"Undefined pure function '{name}'.",
                        ctx=expr,
                ...
                if len(args) != len(signature.params):
                    self._add_diagnostic(
                        ...
                        message=f"Pure function '{name}' expects {len(signature.params)} argument(s), but got {len(args)}.",
                        ctx=expr,
                        hint="Pass the exact number of arguments declared in the pure function signature.",
                    )
                    return signature.return_type
                for index, (arg, expected) in enumerate(
                    zip(args, signature.params), start=1
                ):
                    actual_type = self._resolve_expr_type(arg)
                    if actual_type != expected.declared_type:
                        resolved_actual = (
                        ...
                        self._add_diagnostic(
                            ...
                            message=(
                                f"Type mismatch for argument {index} in pure call '{name}': "
        ...
        def visitVarDecl(self, ctx):
            declared_type_ctx = self._call_ctx(ctx, "typeName")
            declared_type = declared_type_ctx.getText() if declared_type_ctx else None
            if declared_type is not None:
                self._validate_declared_type(declared_type, declared_type_ctx, "LCK310")
            initializer = self._call_ctx(ctx, "expr")
            initializer_type = (
                self._resolve_expr_type(initializer)
            ...
            if declared_type is None:
                self._add_diagnostic(
                    ...
                    message=f"Cannot infer type for local variable '{self._token_text(ctx.ID())}' without a typed initializer.",
                    ctx=ctx,
                    hint="Provide an explicit type or initialize with an expression whose type can be resolved.",
                )
                return self.visitChildren(ctx)
            self._declare(
                self._token_text(ctx.ID()),
                declared_type,
                ctx,
                duplicate_code="LCK306",
                kind="local",
                assigned=initializer is not None,
            )
            if initializer_type is not None and initializer_type != declared_type:
                self._add_diagnostic(
                    ...
                    message=(
                        f"Type mismatch in initializer for '{self._token_text(ctx.ID())}': "
        ...
        def visitAssignStmt(self, ctx):
            lvalue_ctx = self._call_ctx(ctx, "lvalue")
            lvalue_type = self._resolve_lvalue_type(lvalue_ctx, for_write=True)
            expr_type = self._resolve_expr_type(self._call_ctx(ctx, "expr"))
            if (
                ...
                self._add_diagnostic(
                    ...
                    message=f"Type mismatch in assignment: left-hand side expects {lvalue_type}, got {expr_type}.",
                    ctx=ctx,
            ...
            if lvalue_ctx is not None:
                self._set_symbol_path_assigned(
                    tuple(
                        ...
                        for token in self._collect_id_tokens(lvalue_ctx)
                    )
                )
            return self.visitChildren(ctx)

        def visitReturnStmt(self, ctx):
            if self._current_pure_function is not None:
                actual_type = self._resolve_expr_type(self._call_ctx(ctx, "expr"))
        ...
        def visitStructDecl(self, ctx):
            struct_name = self._token_text(ctx.ID())
            if struct_name in self.structs:
                ...
                return self.visitChildren(ctx)
            fields = {}
            for member in self._call_ctx(ctx, "structMember", []) or []:
                field_name = self._token_text(member.ID())
                field_type = member.typeName().getText()
                if field_name in fields:
                    self._add_diagnostic(
                        severity="error",
                        code=SEMANTIC_DIAGNOSTIC_CODES["duplicate_struct_field"],
                        message=f"Struct '{struct_name}' has duplicate field declaration '{field_name}'.",
                        ctx=member,
                        hint="Rename or remove duplicate struct member declarations.",
                    )
                    continue
                fields[field_name] = SemanticStructField(
                    name=field_name, declared_type=field_type
                )
        ...
        def visitPureDecl(self, ctx):
            ...
            params = []
            pure_param_list = self._call_ctx(ctx, "pureParamList")
            if pure_param_list is not None:
                param_types = pure_param_list.typeName()
                param_names = pure_param_list.ID()
                for index, param_name in enumerate(param_names):
                    declared_type = param_types[index].getText()
                    self._validate_declared_type(
                        declared_type, param_types[index], "LCK310"
                    )
                    params.append(
                        SemanticKernelParam(
                            name=self._token_text(param_name),
                            declared_type=declared_type,
                            modifier="value",
                        )
                    )
            self.pure_functions[name] = SemanticPureFunctionSignature(
                return_type=return_type, params=tuple(params)
            )
            statements = []
            statement_method = getattr(ctx, "statement", None)
            if callable(statement_method):
                statement_nodes = statement_method() or []
                statements = (
                    statement_nodes
                    if isinstance(statement_nodes, list)
                    else [statement_nodes]
                )
            if not statements and self._call_ctx(ctx, "returnStmt") is not None:
                statements = [ctx]
            return_statements = []
            for index, statement in enumerate(statements):
                return_stmt = self._call_ctx(statement, "returnStmt")
            ...
            if return_statements:
                ...
                for statement in statements[first_return_index + 1 :]:
                    self._add_diagnostic(
                        severity="warning",
                        code=SEMANTIC_DIAGNOSTIC_CODES["pure_unreachable_after_return"],
                        message=f"Unreachable statement in pure function '{name}' after return statement.",
                        ctx=statement,
                        hint="Remove or move statements before the return.",
                    )
            self._push_scope()
            previous = self._current_pure_function
            ...
            result = self.visitChildren(ctx)
            self._current_pure_function = previous
            self._pop_scope()
            return result
        ...
        def visitPipelineDecl(self, ctx):
            ...
            for resource_name, resource in declared_resources.items():
                ...
                self._add_diagnostic(
                    ...
                    message=f"Pipeline {resource.kind} '{resource_name}' is declared but not used in the bind block.",
        ...
        def visitStreamDecl(self, ctx):
            ...
            if self._pipeline_resource_stack:
                self._pipeline_resource_stack[-1][self._token_text(ctx.ID())] = (
        ...
        def visitAccumDecl(self, ctx):
            ...
            if self._pipeline_resource_stack:
                self._pipeline_resource_stack[-1][self._token_text(ctx.ID())] = (
                    SemanticPipelineResource(kind="accumulator", declaration_ctx=ctx)
                )
            return self.visitChildren(ctx)
        ...
        def visitUniformDecl(self, ctx):
            ...
            initializer = self._call_ctx(ctx, "expr")
            if initializer is not None:
                initializer_type = self._resolve_expr_type(initializer)
                if initializer_type is not None and initializer_type != declared_type:
                    self._add_diagnostic(
                        severity="error",
                        code=SEMANTIC_DIAGNOSTIC_CODES[
                        ...
                        message=(
                            f"Type mismatch in uniform initializer for '{self._token_text(ctx.ID())}': "
                            f"expected {declared_type}, got {initializer_type}."
                        ),
                        ctx=ctx,
        ...
        def visitBindStmt(self, ctx):
            ...
            arg_list = self._call_ctx(ctx, "argList")
            if arg_list is not None:
                target_name = self._token_text(id_tokens[0])
                callee_name = self._token_text(id_tokens[1])
                arg_tokens = []
                arg_id_method = getattr(arg_list, "ID", None)
                if callable(arg_id_method):
                    ids = arg_id_method() or []
                    arg_tokens = ids if isinstance(ids, list) else [ids]
                else:
                    arg_tokens = id_tokens[2:]
                self._type_check_bind_call(
                    ctx,
                    target_name,
                    callee_name,
                    [self._token_text(token) for token in arg_tokens],
                )
                return self.visitChildren(ctx)
            fold_target = self._token_text(id_tokens[0])
            fold_operator = self._call_ctx(ctx, "foldOperator").getText()
            fold_source = self._token_text(id_tokens[1])
            declared_type = ctx.typeName().getText()
            self._validate_declared_type(
                declared_type,
                ctx.typeName(),
            ...
            if fold_source_symbol is None:
                ...
                self._add_diagnostic(
                    severity="error",
                    code=SEMANTIC_DIAGNOSTIC_CODES["fold_unknown_source"],
                    message=f"Fold source '{fold_source}' must reference an accumulator, got {fold_source_symbol.kind}.",
                    ctx=ctx,
